...
class CompletionKwargs(BaseModel):
    temperature: Optional[float] = Field(
        None, description="Sampling temperature between 0 and 2")
...
class OpenRouterPromptTemplate(BaseModel):
    ...
    messages: BasePromptMessages = Field(...,
...
class PromptRequest(BaseModel):
    ...
    system_messages: str = Field(...,
                                 description="System messages for the prompt")
    model: Optional[str] = Field(None, description="Model to use")
    kwargs: CompletionKwargs = Field(
        default_factory=CompletionKwargs, description="Optional extra kwargs for completion")
    response_format: Optional[Union[dict, Type[BaseModel]]] = Field(
        None, description="Response format as a Pydantic BaseModel class for structured output")
